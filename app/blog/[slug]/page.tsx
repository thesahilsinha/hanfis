  ...
  return (
    <div style={{ maxWidth: 800, margin: '0 auto', padding: '48px 24px' }}>
      ...
      <article>
        ...
        {post.content ? (
          <div
            style={{ fontSize: 16, color: '#333', lineHeight: 1.85 }}
      ...
      {related && related.length > 0 && (
        <div style={{ marginTop: 64, paddingTop: 48, borderTop: '1px solid #e8e8e8' }}>
          ...
          <div style={{ display: 'grid', gridTemplateColumns: 'repeat(2,1fr)', gap: 20 }}>
            {related.map(r => (
              <Link key={r.id} href={`/blog/${r.slug}`} style={{ textDecoration: 'none', background: '#fff', borderRadius: 16, overflow: 'hidden', border: '1.5px solid #e8e8e8', transition: 'all .2s' }}
