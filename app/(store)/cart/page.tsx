...
export default function CheckoutPage() {
  ...
  if (order) return (
    <div style={{ maxWidth: 600, margin: '80px auto', padding: '0 16px', textAlign: 'center' }}>
      ...
      <p style={{ color: '#888', marginBottom: 32 }}>We've opened WhatsApp for confirmation.</p>
  ...
  return (
    <div className="page-inner" style={{ paddingTop: 48, paddingBottom: 48 }}>
      ...
      <div className="checkout-grid">
        <div>
          <div style={{ marginBottom: 32 }}>
            ...
            {[['name', 'Full Name *', 'text'], ['phone', 'Phone Number *', 'tel'], ['email', 'Email', 'email'], ['address', 'Delivery Address', 'text']].map(([k, l, t]) => (
          ...
          <div style={{ marginBottom: 32 }}>
            ...
            <div className="payment-btns">
              {settings?.online_payment_enabled !== false && (
                <button onClick={() => setForm({ ...form, payment: 'online' })} className={`payment-btn${form.payment === 'online' ? ' active' : ''}`}>💳 Online Payment</button>
              )}
          ...
          <div style={{ marginBottom: 32 }}>
            ...
            <div style={{ display: 'flex', gap: 12 }}>
              ...
              <button onClick={validateCoupon} className="btn-primary">Apply</button>
            </div>
          ...
          <div style={{ background: '#f9f9f9', borderRadius: 20, padding: 28, position: 'sticky', top: 100 }}>
            ...
            {items.map(item => (
              <div key={item.id} style={{ display: 'flex', gap: 12, marginBottom: 16 }}>
                <img src={item.image} alt={item.name} style={{ width: 56, height: 56, objectFit: 'contain', background: '#fff', borderRadius: 8, padding: 4, flexShrink: 0 }} />
